...
Estos valores quedan confirmados para el MVP y ya están materializados
como `INSERT` defaults en `docs-ruta/bd/ruta_postgres.sql`.

### Plazos de pedido (Flujo 1, Cliente Full)

| Parameter key | Valor confirmado | Unidad | Descripción |
|---|---|---|---|
...
| Parameter key | Valor confirmado | Unidad | Descripción |
|---|---|---|---|
| `cleanup.guest_orphan_minutes` | `10` | minutos | Un comprador invitado que no llegó a hacer ningún pedido se borra pasado este tiempo. **Sin retención**: no deja rastro. Job `cleanup_guest_buyers`, corre cada 5 min |
| `storage.evidence_retention_days` | `14` | días | Vigencia de la foto del recibo guardada en base64. Pasado el plazo se purga y queda la marca `purged_at`, para distinguir «expiró» de «nunca hubo». Job `purge_collection_evidence`, diario a las 03:30 UTC |

### Seguimiento del repartidor

| Parameter key | Valor confirmado | Unidad | Descripción |
|---|---|---|---|
| `tracking.courier_location_ttl_seconds` | `300` | segundos | Cuánto se considera vigente la última posición del repartidor. Pasado el plazo, al comprador se le dice que está desfasada en vez de mostrarla como actual |
