...
| `0.INFRA-4` | [/] Publicacion verificada / PR pendiente | Secret `NPM_PUBLISH_TOKEN` existe; `@orkoruta/shared@1.0.0` y `@orkoruta/db@1.0.0` existen en GitHub Packages; instalacion real desde `/tmp` OK; CI local OK; PR #1 abierto en `packages-ruta` para subir workspace/workflows/script idempotente; CI remoto `test` SUCCESS. Pendiente: review y merge del PR. | `packages-ruta`, GitHub Packages, https://github.com/orkoruta/packages-ruta/pull/1 |
