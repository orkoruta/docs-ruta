...
| `2.STORE-2` | ✅ Lista para PR | Checkout multi-paso storefront. `/c/[slug]/checkout/page.tsx` shell server compatible con `output: export`. `CheckoutStepper.tsx` ('use client'): lee el DRAFT con `getDraftOrder()` desde `cart.api.ts`, guard 401 → `/c/${slug}/login?return=/checkout`, valida carrito/stock, orquesta pasos, confirma `PATCH /buyer/orders/:id/confirm` con `X-Idempotency-Key`; si `ONLINE_AT_ORDER` llama `POST /buyer/orders/:id/initiate-payment` y redirige a `wompi_checkout_url`; si COD redirige a `/c/${slug}/orders/${id}`. `DeliveryStep.tsx`: selección SHIP/PICKUP. `AddressStep.tsx`: dirección SHIP o pickup point con mapa OpenStreetMap/Leaflet cargado client-side por CDN. `PaymentStep.tsx`: Wompi, pago electrónico contra entrega con submétodos y efectivo contra entrega. Verificación: `pnpm --filter @ruta/storefront typecheck` bloqueado por archivos untracked de `2.STORE-3` presentes en este worktree (`orders/page.tsx` y `orders/[id]/page.tsx` importan componentes no presentes); `next lint`/eslint quedaron colgados y se terminaron con SIGTERM. | `frontend-ruta/storefront/src/app/c/[slug]/checkout/*`, rama `feat/store-2-2` |
...
Última actualización: 2026-05-28 — **Sprint 2: Wave 2.3 lista para PR.** 2.STORE-2 ✅ checkout multi-paso en `feat/store-2-2`; 2.STORE-3 ✅ mis pedidos BUYER en `feat/store-2-3`.
Total de archivos generados: 41 + esta memoria = 42.
