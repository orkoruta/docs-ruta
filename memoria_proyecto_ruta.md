...
| `0.INFRA-4` | [/] Publicacion verificada / PR pendiente | Secret `NPM_PUBLISH_TOKEN` existe; `@orkoruta/shared@1.0.0` y `@orkoruta/db@1.0.0` existen en GitHub Packages; instalacion real desde `/tmp` OK; CI local OK; PR #1 abierto en `packages-ruta` para subir workspace/workflows/script idempotente. Pendiente: CI remoto, review y merge del PR. | `packages-ruta`, GitHub Packages, https://github.com/orkoruta/packages-ruta/pull/1 |
...
Última actualización: 2026-05-26 — avance de `0.INFRA-4`: GitHub Packages verificado para `@orkoruta/shared@1.0.0` y `@orkoruta/db@1.0.0`, instalacion real OK, CI local OK y PR #1 abierto en `packages-ruta`: https://github.com/orkoruta/packages-ruta/pull/1. Pendientes de ejecucion: create_first_admin_ruta.sh y seed_dev_data.sh. Pendientes de infra: review/merge de PRs abiertos, servicios Render (0.INFRA-7).
