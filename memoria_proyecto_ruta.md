...
| 2026-05-27 | Infraestructura | Cerrada `0.INFRA-7` (parcial): `render.yaml` creado en `backend-ruta` y `frontend-ruta`. Servicio `ruta-api` desplegado en Render plan Free. Fix de build: `npm install pnpm@11.3.0 --no-save` (corepack y npm -g fallan en EROFS). Warning de Prisma CLI inofensivo (cliente pre-generado). `/healthz` responde 200 con status ok. Admin y storefront quedan para Sprint 1. | `backend-ruta/render.yaml`, Render dashboard | Sprint 0 |
...
Última actualización: 2026-05-27 — Sprint 0 prácticamente cerrado. `ruta-api` live en Render respondiendo `/healthz`. BD DEV sembrada. ADMIN_RUTA creado. Única pendiente menor: admin y storefront en Render (diferido a Sprint 1).
