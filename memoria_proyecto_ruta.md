...
| 2026-05-27 | Infraestructura | Completada parte ejecutable de `0.BACK-1`, `0.FRONT-1` y `0.INFRA-7`: verificado que `backend-ruta` y `frontend-ruta` ya resolvían `@orkoruta/*` desde GitHub Packages (no file:); typecheck+tests OK en ambos. Agregados `render.yaml` en `backend-ruta` (servicios ruta-api + ruta-api-worker) y `frontend-ruta` (ruta-admin + ruta-storefront). Corregidos `.npmrc` con `${NPM_TOKEN}` para auth en Render. Scripts `start` de Next.js usan `${PORT:-30xx}`. Script `start:worker` agregado como placeholder. Pusheado a `main` en ambos repos. Pendiente: crear los servicios en Render dashboard (New Blueprint → conectar repo → setear secretos: DATABASE_URL, JWT_SECRET, NPM_TOKEN, Wompi keys). | `backend-ruta/render.yaml`, `frontend-ruta/render.yaml`, `.npmrc` en ambos repos | Sprint 0 |
...
Última actualización: 2026-05-27 — `0.INFRA-4` cerrada: PR #1 de `packages-ruta` mergeado. CI workflows pusheados a `backend-ruta` y `frontend-ruta`. Pendientes Sprint 0: `0.BACK-1` y `0.FRONT-1` (cambiar file: a GitHub Packages), `0.INFRA-7` (Render), `0.BACK-2` y `0.INFRA-8` (requieren psql).
